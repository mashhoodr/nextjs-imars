 ...
 *   video       optional, a click-to-load external video. Shape and rationale
 *               are documented in components/linkedin-video.js; read by
 *               videoOf() below
 */
...
/**
 * An externally-hosted video, rendered as a click-to-load facade rather than a
 * live embed. See components/linkedin-video.js for why, and for the frontmatter
 * contract.
 *
 * The poster's dimensions are read off the file here, at build time, for the
 * same reason withImageDimensions() does it below: attributes typed by hand
 * drift from the asset, and a poster without a reserved box is a layout shift.
 */
function videoOf(data) {
  const v = data.video
  if (!v || !v.embedUrl || !v.url) return null

  let poster = null
  if (v.poster) {
    const size = imageSize(path.join(process.cwd(), 'public', v.poster))
    // No dimensions means the file is missing or unreadable. Dropping the
    // poster is better than shipping an unsized image that shifts the page.
    if (size) poster = { src: v.poster, width: size.w, height: size.h }
  }
